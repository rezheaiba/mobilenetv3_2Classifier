...
def main():
    ...
    best_acc = 0.0
    # best_acc = 0.98
    save_path_best = 'weight-v3-JP/model_v3_best.pth'
    save_path_newest = 'weight-v3-JP/model_v3_newest.pth'
    train_steps = len(train_loader)
    val_steps = len(validate_loader)
    prev_time = time.time()

    # creat logger 用来保存训练以及验证过程中信息
    logger_file = "./weight-v3-JP/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(epochi, epochs):
        ...
        with torch.no_grad():
            ...
            for step, val_data in enumerate(val_bar):
                ...
                acc += torch.eq(predict_y, val_data['label'].to(device)).sum().item()
