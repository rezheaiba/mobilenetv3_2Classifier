...
def main():
    ...
    print("using {} images for training, {} images for validation.".format(train_num,
                                                                           val_num))
    net = MobileNetV3(dilated=True, num_classes=8, arch='mobilenet_v3_small', reduced_tail=True, width_mult=0.3)
    net.to(device)
    eff_net = efficientnet_lite_tiny(8).to(device)
    eff_net.load_state_dict(torch.load('weight-efficientnet/model_v3_newest.pth', map_location=device))
    eff_net.eval()
    ...
    best_acc = 0.0
    # best_acc = 0.98
    save_path_best = 'weight-efficientnet-kd/model_v3_best.pth'
    save_path_newest = 'weight-efficientnet-kd/model_v3_newest.pth'
    train_steps = len(train_loader)
    val_steps = len(validate_loader)
    prev_time = time.time()

    # creat logger 用来保存训练以及验证过程中信息
    logger_file = "./weight-efficientnet-kd/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(epochi, epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            outputs_teacher = eff_net(images.to(device))
